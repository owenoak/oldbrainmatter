...
new Class("List", {
	...
	properties : {
		// on init, add arguments to the list
		init : function(items) {
			if (items && items.length) this.addList(items);
		},
...
// flag so we can tell if we're dealing with a list-like thing
...
List.makeAccessors = function(constructor, properties, firstOnly) {
	...
	while (method = properties[++index]) {
		Object.defineProperty(proto, method, new Descriptor({

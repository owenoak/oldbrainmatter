...
// figure out if we're in a environment we can support
...
// show a very severe message if this environment is not supported
...
var hope = {
	toString : function(){ return "[hope]"},
	
	//! environment name == "Firefox", "WebKit", "Chrome" or "Unknown"
	environment : {
	...
	/** Add all properties from one or more objects to the first object.
		Very quick, does not getter/setter checking.
	...
	*/
	...
	/** Simple shallow merge for one or more property object.
		...
		Sets up getters ("get:property") and setters ("set:property") automatically.
	...
	mixin : function mixin() {
		...
		while (++i < length) {
			...
			for (key in source) {
				...
				// getter?
				if (beginning == "get:" && typeof value === "function") {
					destination.__defineGetter__(key.substr(4), value);
					continue;
				}
	
				// setter?
				if (beginning == "set:" && typeof value === "function") {
					if (key == "setUNKNOWN") {
	...
	*/
	...
	/** Given a list of objects (eg: a list of children from childrenToObjects, etc)
		convert to a map of arrays as key->list for property key of each object.
		Ignores things that don't have the key.
	*/
	listToMap : function(list, key, map) {
		map = map || {};
		var i = 0, it;
		while (it = list[i++]) {
			var value = it[key];
	...
	expand : function expand(string, context) {
		var match, pattern = hope._expandPattern, replacement;
	...
	*/
	...
	loadScripts : function loadScripts(urls, onload) {
		...
		if (hope.environment.executesScriptsInOrder) {
			function loadedOne() {
	...
	*/
	...
	/** Register a Thing so you can get it back with hope.getThing(). */
...
hope.extend(hope, {
	...
	*/
	...
	//	hope.ajax(url, onload, {onerror=hope.error, type='text', method=GET, cache=false, async=true})	=> request
	ajax : function ajax(url, onload, options) {
		if (!options) options = {};

		// transform with hope.url, applying timestamp if told to
		var massagedUrl = hope.url(url, (options.cache != true));
		var method = options.method || "GET";
		var async = (options.async === undefined ? true : options.async != false);

		// make the actual request
		var request = new XMLHttpRequest();
		request.open(method, massagedUrl, async);
		if (onload) {
			var responseData = (options.type == "xml" ? "responseXML" : "responseText");
			// TODO: handle other response types
			request.onload = function() {
				onload(request[responseData], url, request);
			}
		}
...
// 'export' hope if we're in CommonJS-land
if (global.exports) global.exports.hope = hope;

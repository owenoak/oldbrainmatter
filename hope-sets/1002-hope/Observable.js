// TODO:
//	- verify
//		- construction pattern with "on:" properties
//		- passing/inheritance behavior
//		- ignore(target) works
//
//	- bindTo (as in constructor pattern)
//
	...
	all : {
		...
		*/
		observe : function(event, observation, when) {
			...
			var observations = (this.hasOwnProperty("observations") 
									? this.observations 
									: this.observations = hope.protoClone(this.Class.prototype.observations || {})
							   );
		...
		*/
		ignore : function(event, target) {
			...
			if (target == null) {
				this.observations[event] = this.Class.prototype.observations[event];
			} else {

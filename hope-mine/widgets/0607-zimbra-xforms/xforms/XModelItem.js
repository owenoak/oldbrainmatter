...
XModel.registerErrorMessage("valueIsRequired", "Value is required");
XModelItem.prototype.validate = function (value, form, formItem, instance) {
	...
	if (!constraints.length) constraints = [constraints];
...
XModelItem.prototype.getPattern = function() {
	if (this.pattern != null && this.pattern.checked == null) {
		...
		else if (typeof this.pattern == "regexp") {		//CONVERSION?
...
XModel.registerErrorMessage("notAString",		XFormsMsg.notAString);
XModel.registerErrorMessage("stringLenWrong",   XFormsMsg.stringLenWrong);
XModel.registerErrorMessage("stringTooShort", 	XFormsMsg.stringTooShort);
XModel.registerErrorMessage("stringTooLong",	XFormsMsg.stringTooLong);
XModel.registerErrorMessage("stringMismatch",   XFormsMsg.stringMismatch);

XModelItem.prototype.validateString = function(value) {
	if (value == null) return;
	
	if (!XFormsUtil.isString(value)) {
...
XModel.registerErrorMessage("notANumber",				XFormsMsg.notANumber);
XModel.registerErrorMessage("numberTotalExceeded", 		XFormsMsg.numberTotalExceeded);
XModel.registerErrorMessage("numberFractionExceeded", 	XFormsMsg.numberFractionExceeded);
XModel.registerErrorMessage("numberMoreThanMax", 		XFormsMsg.numberMoreThanMax);
XModel.registerErrorMessage("numberMoreThanEqualMax", 	XFormsMsg.numberMoreThanEqualMax);
XModel.registerErrorMessage("numberLessThanMin", 		XFormsMsg.numberLessThanMin);
XModel.registerErrorMessage("numberLessThanEqualMin", 	XFormsMsg.numberLessThanEqualMin);
...
XModelItem.prototype.validateNumber = function(value) {
	...
	if (isNaN(nvalue) || isNan(parseFloat(value))) {
...
XModel.registerErrorMessage("invalidDateString", XFormsMsg.invalidDateString);

// methods
XModelItem.prototype.validateDate = function(value) {
	
	if (XFormsUtil.isInstance(value, Date)) return value;
	if (XFormsUtil.isString(value)) {
		...
		if (value.indexOf("/") > -1) {
			...
			if (value == XFormsMsg.today) {
				return date;
			} else if (value == XFormsMsg.yesterday) {
				date.setTime(date.getTime() - this.msecInOneDay);
				return date;
			} else if (value == XFormsMsg.tomorrow) {
				date.setTime(date.getTime() + this.msecInOneDay);
				return date;
			}
		}
...
XModel.registerErrorMessage("invalidTimeString",		 XFormsMsg.invalidTimeString);
// time is returned as a number of milliseconds since
XModelItem.prototype.validateTime = function (value) {

	if (XFormsUtil.isNumber(value)) return value;
	
	if (XFormsUtil.isInstance(value, Date)) {
...
XModel.registerErrorMessage("invalidDatetimeString",		 XFormsMsg.invalidDatetimeString);
...
XModel.registerErrorMessage("didNotMatchChoice",	XFormsMsg.didNotMatchChoice);
...
Enum_XModelItem.prototype.validateType = function (value) {
	...
	for (var i = 0; i < choices.length; i++) {
		...
		if (XFormsUtil.isInstance(choice, Object)) {
...
FileSize_XModelItem.prototype.units = XFormsUtil.SIZE_MEGABYTES;
...
FileSize_XModelItem.prototype.getValue =  function(ins, current, ref) {
	...
	return value ? XFormsUtil.formatSizeForUnits(value, XFormsUtil.SIZE_KILOBYTES, false, 2) : 0;
}

FileSize_XModelItem.prototype.setValue = function(value, ins, current, ref) {
	return eval("ins."+ref+" = XFormsUtil.parseSize(value, this.units)");
}
...
//HostNameOrIp_XModelItem.prototype.pattern = [ XFormsUtil.HOST_NAME_RE, XFormsUtil.IP_ADDRESS_RE,XFormsUtil.HOST_NAME_WITH_PORT_RE ];
...
//ShortURL_XModelItem.prototype.pattern = [XFormsUtil.SHORT_URL_RE,XFormsUtil.IP_SHORT_URL_RE];

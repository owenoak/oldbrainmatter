...
//console.info("key:"+node.tagName+" = ",value," was ",$(node).text());
				}
				// otherwise if an element, it's value is a recursive call to $.xml.toObject()
				else {
//console.info("recursing for key :"+node.tagName);
					// start the nested object out with any attributes on the node
					var nestedObject = $.xml.getAttributes(node);
					value = $.xml.toObject(node, null, nestedObject);
				}
...
//console.info(key+":adding direct ",value);
					object[key] = value;
				}
...
//console.info(key+":adding to array:",currentValue);
						currentValue.push(value);
					} else {
//console.info(key+":creating array w/",currentValue);
		...
		*/
		arrayToObject : function(array, keyField, valueField, object, parse, trim) {
			...
			if (array && array.length) {
				...
				if (valueField == null) valueField || "name";
				if (parse == null) parse = true;
				if (trim == null) trim = true;
				Array.forEach(array, function(it) {
					var key = it[keyField],
						value = it[valueField]
					;
